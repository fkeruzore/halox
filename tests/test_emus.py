...
def test_sigmaM_emu(cosmo_name, n_k_int, return_vals=False):
    ...
    tinker08_f_sigma = jax.jit(
        lambda m, z: halox.lss.sigma_M(m, z, cosmo=cosmo_j, n_k_int=n_k_int)
    )
    f_c = jnp.array(
        [
    ...
    tinker08_f_sigma_emu = jax.jit(lambda m, z: emu(m, z, cosmo_ray=cosmo_j))
...
def test_tinker10_bias(delta_c, cosmo_name, return_vals=False):
    ...
    b_emu = jnp.array(
        [
            halox.bias.tinker10_bias(
                ms[i], zs[i], cosmo=cosmo_j, delta_c=delta_c, emulate=True
            )
            for i in range(len(test_mzs))
        ]
    )
    b_h = jnp.array(
        [
            tinker10_bias(
                ms[i], zs[i], cosmo=cosmo_j, delta_c=delta_c, emulate=False
            )
            for i in range(len(test_mzs))
        ]
    )
