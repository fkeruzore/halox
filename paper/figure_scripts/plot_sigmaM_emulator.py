...
for label, color, sig_a, sig_e, sig_c in results:
    ...
    ax_top.loglog(
        masses_np, sig_c, color=color, ls=":", lw=2, label=f"{label}, colossus"
    )

    diff_a = (sig_a / sig_c - 1) * 100
    diff_e = (sig_e / sig_c - 1) * 100
    ax_bot.semilogx(masses_np, diff_a, color=color, ls="-", lw=1.5)
    ax_bot.semilogx(masses_np, diff_e, color=color, ls="--", lw=2)
