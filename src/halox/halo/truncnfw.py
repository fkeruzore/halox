...
class TruncNFWHalo:
    """
    Properties of a dark matter halo following a truncated Navarro-Frenk-White
    density profile.
    ...
    delta: float
        ...
        defaults to 200.
    r_trunc: float
        tidal radius (need more description)
    """
    ...
    def __init__(
        ...
        self.r_trunc = jnp.asarray(r_trunc)
    ...
    def density(self, r: ArrayLike) -> Array:
        ...
        return (
            self.rho0
            / (r / self.Rs * (1 + r / self.Rs) ** 2)
            * (self.r_trunc**2 / (r**2 + self.r_trunc**2))
        )
